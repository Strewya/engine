...
CReadChangesRequest::CReadChangesRequest(CReadChangesServer* pServer, const std::string& sz, BOOL b, DWORD dw, DWORD size)
{
	m_pServer		= pServer;
	m_dwFlags		= dw;
	m_bChildren		= b;
...
void CReadChangesRequest::ProcessNotification()
{
	...
	for (;;)
	{
		...
		*/
		m_pServer->m_pBase->Push(fni.Action, fileName);
