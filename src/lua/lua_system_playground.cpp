...
namespace core
{
   ...
   void test_luaStack()
   {
      ...
      CORE_ASSERT_ERR(stack.getTop() == 0, "Playground error");

      auto loaded = stack.loadFile(CORE_RESOURCE("Defs/hedgehog.shee"));
      CORE_ASSERT_ERR(!loaded, "Playground error");
      CORE_ASSERT_ERR(stack.is<str>(), "Playground error");
      ...
      CORE_ASSERT_ERR(loaded, "Playground error");
      CORE_ASSERT_ERR(stack.is<LuaFunction>(), "Playground error");

      auto called = stack.call();
      CORE_ASSERT_ERR(called, "Playground error");
      CORE_ASSERT_ERR(stack.is<LuaTable>(), "Playground error");
      stack.pop(1); //table
      CORE_ASSERT_ERR(stack.getTop() == 0, "Playground error");
      ...
      {
         CORE_ASSERT_ERR(stack.is<str>(-2), "Playground error");
         CORE_ASSERT_ERR(stack.is<LuaTable>(-1), "Playground error");
      ...
      for( stack.pairs("images"); stack.next(); stack.pop(1) )
      {
         ...
         CORE_ASSERT_ERR(stack.is<LuaTable>(), "Playground error");
         stack.pull(1);
         CORE_ASSERT_ERR(stack.is<i32>() || stack.is<nullptr_t>(), "Playground error");
         if( stack.is<nullptr_t>() )
         {
            stack.pop(1); //nil
            stack.pull("x");
            CORE_ASSERT_ERR(stack.is<i32>(), "Playground error");
         }
         f32 x = stack.to<f32>();
         CORE_LOG_DEBUG("x: ", x);
         stack.pop(1); //number
         ...
         stack.pop(1); //number
         stack.pull(3);
         CORE_ASSERT_ERR(stack.is<nullptr_t>(), "Playground error");
         stack.pop(2); //nil and 'pos' table
         stack.pull("height");
         CORE_ASSERT_ERR(stack.is<nullptr_t>() || stack.is<u32>(), "Playground error");
         if( stack.is<u32>() )
         {
      ...
      CORE_ASSERT_ERR(stack.getTop() == 1, "Playground error");
      ...
      for( stack.pairs("animations"); stack.next(); stack.pop(1) )
      {
         ...
         CORE_LOG_DEBUG("table name: ", stack.to<str_writeable>(-2));
         stack.pull("loop");
         CORE_ASSERT_ERR(stack.is<nullptr_t>() || stack.is<bool>(), "Playground error");
         if( stack.is<bool>() )
         {
         ...
         stack.pull("duration");
         CORE_ASSERT_ERR(stack.is<nullptr_t>() || stack.is<f32>(), "Playground error");
         if( stack.is<f32>() )
         {
         ...
         {
            CORE_ASSERT_ERR(stack.is<u32>(-2), "Playground error");
            CORE_ASSERT_ERR(stack.is<str>(-1), "Playground error");
      ...
      for( stack.pairs(); stack.next(); stack.pop(1) )
      {
         ...
         stack.pull("size");
         CORE_ASSERT_ERR(stack.is<u32>(), "Playground error");
      ...
      CORE_ASSERT_ERR(stack.is<i32>(), "Playground error");
      i32 n = stack.to<i32>();
      CORE_ASSERT_ERR(n == 1, "Playground error");
      stack.pop(1);
      CORE_ASSERT_ERR(stack.getTop() == 0, "Playground error");
      ...
      CORE_ASSERT_ERR(stack.is<i32>(), "Playground error");
      n = stack.to<i32>();
      CORE_ASSERT_ERR(n == 5 + 2, "Playground error");
      stack.pop(1);
      CORE_ASSERT_ERR(stack.getTop() == 0, "Playground error");
      ...
      CORE_ASSERT_ERR(s == "are you gay? yes", "Playground error");
      stack.pop(1);
      CORE_ASSERT_ERR(stack.getTop() == 0, "Playground error");
      ...
      CORE_ASSERT_ERR(called, "Playground error");
      CORE_ASSERT_ERR(stack.getTop() == 0, "Playground error");
      CORE_ASSERT_ERR(a == x && b == y && c == z, "Playground error");
      /*
            stack.pull("test5", 0);
      ...
      CORE_ASSERT_ERR(called, "Playground error");
      CORE_ASSERT_ERR(s == "are you a monkey? yes", "Playground error");
      CORE_ASSERT_ERR(stack.getTop() == 0, "Playground error");
