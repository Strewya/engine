...
namespace Core
{
   bool GuiSystem::init(Vec2f windowSize)
   {
      bool status = true;
   ...
   bool GuiSystem::Button::handleEvent(const WindowEvent& we, Vec2f windowSize)
   {
      if( we.m_type == WE_MOUSEBUTTON &&
         we.m_mouseButton.m_isDown &&
         we.m_mouseButton.m_button == activator )
      {
         Vec2f clickedPoint{(float)we.m_mouseButton.m_x, (float)we.m_mouseButton.m_y};
   ...
   void GuiSystem::panel(std::string name, std::string parent, Vec2f pos, Vec2f halfSize, Color color)
   {
   ...
   void GuiSystem::button(std::string name, std::string parent, Vec2f pos, Vec2f halfSize, Color color, Mouse::Keys activator, OnClickFunction onClick)
   {
      auto uniqueName = filterFind(m_elements, [&](const std::unique_ptr<GuiElement>& ge) { return ge->name == name; });
      if( uniqueName == m_elements.size() )
      {
   ...
   void GuiSystem::label(std::string name, std::string parent, uint32_t font, std::string text, Vec2f pos, Vec2f scale, Color color, TextJustification justification, bool italic)
   {
      auto uniqueName = filterFind(m_elements, [&](const std::unique_ptr<GuiElement>& ge) { return ge->name == name; });
      if( uniqueName == m_elements.size() )
      {
   ...
   std::string GuiSystem::getLabelText(std::string name)
   {
      auto uniqueName = filterFind(m_elements, [&](const std::unique_ptr<GuiElement>& ge) { return ge->name == name; });
      if( uniqueName != m_elements.size() )
      {
