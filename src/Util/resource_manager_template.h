...
namespace Core
{
   ...
   public:
      ...
      HANDLE loadFromFile(const std::string& filename, Args&&... rest);
   ...
   template<typename DATA, typename HANDLE, typename LOADER>
   template<typename ...Args>
   HANDLE Manager<DATA, HANDLE, LOADER>::loadFromFile(const std::string& filename, Args&&... rest)
   {
   ...
   template<typename DATA, typename HANDLE, typename LOADER>
   HANDLE Manager<DATA, HANDLE, LOADER>::loadFromFile(const std::string& filename)
   {
      HANDLE handle = m_names.getHandle(filename);
      if( handle.isNull() )
      {
