...
#undef not_available
   ...
   core_internal void moveEntities(ComponentMovement& movement, const ComponentDeltaTime& deltaTimes)
   {
      ...
      for( auto i = 0U; i < count; ++i )
      {
         ...
         CORE_ASSERT_DBGERR(iid != InvalidDataId, "Moveable entity [id=", e, "] lacks a DeltaTime component!");
      ...
      for( auto i = 0U; i < count; ++i )
      {
         for( auto j = i + 1; j < count; ++j )
         {
               ...
               collision.m_targetTypeBits[i], collision.m_targetTypeBits[j]) )
            {
               ...
               CORE_ASSERT_DBGERR(iid1 != InvalidDataId, "Colliding entity [id=", eid1, "] has no transform");
               CORE_ASSERT_DBGERR(iid2 != InvalidDataId, "Colliding entity [id=", eid2, "] has no transform");
